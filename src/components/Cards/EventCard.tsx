...
function EventCard({ title, location, imageURL, link, scale }: EventCardProps) {
  title = "Pool Party";
  location = "Köln-Str 5";
  ...
  return (
    <div
      style={{ transform: `scale(${scale})` }} // Apply the scale prop here
    >
      <Link to={link} className="block">
        <main className="h-63 w-60 cursor-pointer flex-col rounded-2xl bg-white pt-0.75 text-left transition-transform hover:scale-102 hover:shadow-lg active:scale-95">
          <img
            src={imageURL}
